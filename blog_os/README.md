...
- 変数
  - ローカル変数：コールスタックに格納される。関数やブロックが終わるまでしか生存しない。
  - スタティック変数：スタックとは別の固定されたメモリ位置に格納される。'static ライフタイムをもっているのでプログラムの実行中ずっと生存する。
  - 動的メモリ
    - 実行時に動的にサイズが変わる変数を格納するためにヒープを使う。allocateでヒープ領域を確保し、deallocateでヒープ領域を解放する
    - メモリリーク：ヒープ領域の解放漏れにより永久に解放されない領域ができて、過剰なメモリ消費を引き起こす
    - use-after-free脆弱性：変数に対してdeallocateを呼んだ後にも間違って使いつづけると未定義動作を引き起こす。しばしば攻撃者が任意コードを実行するのに利用される
    - double-free脆弱性：間違ってある変数を二度解放すると、メモリ管理に異常な状態を引き起こす。攻撃者の攻撃（メモリ破壊、不正なデータへのアクセス、任意のコードの実行など）を引き起こす可能性がある
    - JavaやPythonなど多くの言語ではガベージコレクションを使って自動的に動的メモリを管理している。未使用のヒープ変数を自動的に解放する。上記の脆弱性が発生しなくなるが、パフォーマンスのオーバーヘッドがかかる。
- マルチタスク
  - OSの基本的な機能の１つで、複数のタスクを同時に実行する機能
  - CPUコアで一度に実行できるタスクは１つだけで、並行して実行されているように見せるため、OSはアクティブなタスクを迅速に切り替えている
  - シングルコアCPUは一度に１つのタスクしか実行できないが、マルチコアCPUは複数のタスクを真に並行実行できる
  - マルチタスクには、協調マルチタスク（タスク切り替えをタスク制御で実施）とプリエンプティブマルチタスク（タスク切り替えをOS制御で実施）の２つの形式がある
- async/awaitパターン
  - プログラマが通常の同期コードのように見えるコードを書き、コンパイラによって非同期コードに変換されることで、非同期コードを書くことを簡単にする。クロージャや`Future`型を直接利用しなくてもよくなる
  - `async`キーワードを関数シグネチャで使用すると、同期関数を`Future`を返す非同期関数に変換してくれる
  - `await`キーワードを`async`関数内で利用することで、`Future`の非同期の値を取得できる
  - コンパイラは、`async`関数の本体をステートマシンに変換し、各呼び出しを異なる状態で表している
...
- メモリ管理
  - ページング
    ...
    - ページテーブルにある物理フレームにアクセスするための様々なテクニックを学ぶ
    - ページテーブルに新しいマッピングを作る実装をする
  - ヒープ割り当て
    - 動的メモリの基礎を学ぶ
    - アロケーションインターフェイスを実装してヒープメモリ領域を作成する
    - 独自のヒープアロケータ（バンプアロケータ、リンクリストアロケータ）を実装する
- マルチタスク
  - Async / Await
    - Rustのasync/await機能について知る（Future、Pin、Poll、Executor, Waker)
    - 非同期キーボードタスクと基本的なExecutorを実装して、async/awaitの基本サポートをカーネルに追加
